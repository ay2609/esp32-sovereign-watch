...
bool spi_master_write_color(TFT_t * dev, colorSize color, uint16_t size) {
    static uint8_t Byte[1024]; // size*3 must be <= this buffer size
    int index = 0;
    for(int i = 0; i < size; i++) {
        Byte[index++] = (color >> 16) & 0xFF;  // R
        Byte[index++] = (color >> 8) & 0xFF;   // G
        Byte[index++] = color & 0xFF;          // B
    }
    gpio_set_level(dev->_dc, SPI_Data_Mode);
    return spi_master_write_byte(dev->_SPIHandle, Byte, size * 2);
}

bool spi_master_write_colors(TFT_t * dev, colorSize * colors, uint16_t size) {
    static uint8_t Byte[1024]; // buffer for up to 512 pixels (3 bytes per pixel)
    int index = 0;
    for (int i = 0; i < size; i++) {
        colorSize color = colors[i];  // expect 0xRRGGBB format
        Byte[index++] = (color >> 16) & 0xFF;  // Red
        Byte[index++] = (color >> 8) & 0xFF;   // Green
        Byte[index++] = color & 0xFF;          // Blue
    }
    gpio_set_level(dev->_dc, SPI_Data_Mode);
    return spi_master_write_byte(dev->_SPIHandle, Byte, size * 2);
}
...
// y:Y coordinate
// color:color
void lcdDrawPixel(TFT_t * dev, int16_t x, int16_t y, colorSize color){
...
// colors:colors
void lcdDrawMultiPixels(TFT_t * dev, uint16_t x, uint16_t y, uint16_t size, colorSize * colors) {
...
// y2:End Y coordinate
// color:color
void lcdDrawFillRect(TFT_t * dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, colorSize color) {
...
// size:Square size
// color:color
void lcdDrawFillSquare(TFT_t * dev, uint16_t x0, uint16_t y0, uint16_t size, colorSize color) {
	uint16_t x1 = x0-size;
	uint16_t y1 = y0-size;
	uint16_t x2 = x0+size;
	uint16_t y2 = y0+size;
...
// Fill screen
// color:color
void lcdFillScreen(TFT_t * dev, colorSize color) {
...
// y2:End	Y coordinate
// color:color 
void lcdDrawLine(TFT_t * dev, int16_t x1, int16_t y1, int16_t x2, int16_t y2, colorSize color) {
	int i;
	int dx,dy;
	int sx,sy;
	int E;
...
void lcdDrawLineAngle(TFT_t * dev, int16_t x1, int16_t y1, int16_t x2, int16_t y2,
                      uint16_t xc, uint16_t yc, int16_t angle, colorSize color) {

    double xd1, yd1, xd2, yd2;
    double rd;

    rd = -angle * M_PI / 180.0; // 0
    xd1 = (int)( x1 * cos(rd) -  y1 * sin(rd) + xc); // x1 + xc
    yd1 = (int)( x1 * sin(rd) +  y1 * cos(rd) + yc); // y1 + yc

//    xd1 = 0;
//    yd1 = 0;

    xd2 = (int)( x2 * cos(rd) - y2 * sin(rd) + xc); // x2 + xc
    yd2 = (int)( x2 * sin(rd) + y2 * cos(rd) + yc); // y2 + yc

//    ESP_LOGI("LineAngle", "(%f, %f) -> (%f, %f)", xd1, yd1, xd2, yd2);

    lcdDrawLine(dev, (int) xd1, (int) yd1, (int) xd2, (int) yd2, color);
}
...
// y2:End	Y coordinate
// color:color
void lcdDrawRect(TFT_t * dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, colorSize color) {
...
void lcdDrawRectAngle(TFT_t * dev, uint16_t xc, uint16_t yc, uint16_t w, uint16_t h, int16_t angle, colorSize color) {
	double xd,yd,rd;
	int x1,y1;
	int x2,y2;
	int x3,y3;
...
int ymin(int y1, int y2, int y3) {
    return (y1 < y2) ? (y1 < y3 ? y1 : y3) : (y2 < y3 ? y2 : y3);
}

// Helper function to find the maximum y-coordinate (top of the triangle)
int ymax(int y1, int y2, int y3) {
    return (y1 > y2) ? (y1 > y3 ? y1 : y3) : (y2 > y3 ? y2 : y3);
}

// Helper function to find the x-coordinate of the intersection of a line with a given y-coordinate
...
int findIntersection(coord* p1, coord* p2, int y) {
    if (p1->y == p2->y) return p2->x; // Handle horizontal line case
    return p1->x + (y - p1->y) * (p2->x - p1->x) / (p2->y - p1->y);  // Linear interpolation formula
}

void sortCoordinates3(coord *a, coord *b, coord *c) {
    coord temp;

    // Ensure a has the smallest y
    if (a->y > b->y) { temp = *a; *a = *b; *b = temp; }
    if (a->y > c->y) { temp = *a; *a = *c; *c = temp; }

    // Ensure c has the largest y
    if (b->y > c->y) { temp = *b; *b = *c; *c = temp; }
}

void sortCoordinates4(coord *a, coord *b, coord *c, coord *d) {
    coord temp;

    // Ensure a has the smallest y
    if (a->y > b->y) { temp = *a; *a = *b; *b = temp; }
    if (c->y > d->y) { temp = *c; *c = *d; *d = temp; }
    if (a->y > c->y) { temp = *a; *a = *c; *c = temp; }
    if (b->y > d->y) { temp = *b; *b = *d; *d = temp; }
    if (b->y > c->y) { temp = *b; *b = *c; *c = temp; }
}

void sortPoints4(int *a, int *b, int *c, int *d) {
    int temp;
...
void lcdDrawTriangle(TFT_t * dev, uint16_t xc, uint16_t yc, uint16_t w, uint16_t h, uint16_t angle, colorSize color) {
	double xd,yd,rd;
	int x1,y1;
	int x2,y2;
	int x3,y3;
    ...
    sortCoordinates3(&p1, &p2, &p3);


#if 1
    // Fill the triangle with color

    for (int y = ymin(y1, y2, y3); y <= ymax(y1, y2, y3); y++) {
        // Find the x-coordinate at the left and right intersections of the horizontal line

        int sect1 = findIntersection(&p1, &p3, y);
        int sect2 = findIntersection(&p1, &p2, y);
        int sect3 = findIntersection(&p2, &p3, y);

        int leftX = (sect1 >= sect2 && sect1 >= sect3) ? ( (sect2 > sect3) ? (sect2) : (sect3) ) : ( (sect2 > sect3) ? (sect3) : (sect2) );
        int rightX = sect1;
        ...
        // Draw the pixels between leftX and rightX at this y-coordinate
...
//    lcdDrawPixel(dev, x1, y1, RED);
//    lcdDrawPixel(dev, x2, y2, BLUE);
//    lcdDrawPixel(dev, x3, y3, GREEN);

}

void lcdDrawTriangleAtAngle(TFT_t * dev, int16_t x1, int16_t y1,
                            int16_t x2, int16_t y2,
                            int16_t x3, int16_t y3,
                            uint16_t xc, uint16_t yc,
                            int16_t angle, colorSize color) {
    ...
    xd1 = (int)(x1 * cos(rd) - y1 * sin(rd) + xc);
    yd1 = (int)(x1 * sin(rd) + y1 * cos(rd) + yc);

    xd2 = (int)(x2 * cos(rd) - y2 * sin(rd) + xc);
    yd2 = (int)(x2 * sin(rd) + y2 * cos(rd) + yc);

    xd3 = (int)(x3 * cos(rd) - y3 * sin(rd) + xc);
    yd3 = (int)(x3 * sin(rd) + y3 * cos(rd) + yc);

    lcdDrawLine(dev, xd1, yd1, xd2, yd2, color);
    lcdDrawLine(dev, xd1, yd1, xd3, yd3, color);
    lcdDrawLine(dev, xd2, yd2, xd3, yd3, color);
...
void lcdDrawFillRectAngle(TFT_t * dev, uint16_t xc, uint16_t yc,
    ...
    sortCoordinates4(&p1, &p2, &p3, &p4);

    for (int y = p1.y; y < p4.y; y++) {

        int sect1 = findIntersection(&p1, &p2, y);
        int sect2 = findIntersection(&p1, &p3, y);
        int sect3 = findIntersection(&p2, &p4, y);
        int sect4 = findIntersection(&p3, &p4, y);

        sortPoints4(&sect1, &sect2, &sect3, &sect4);

        // Draw the pixels between leftX and rightX at this y-coordinate

//        ESP_LOGI("Rectangle", "%d %d %d %d", sect1, sect2, sect3, sect4);

        if (y < p1.y + 2 || y > p4.y - 2 || abs(sect2 - sect3) > 2) {
            for (int x = sect2; x <= sect3; x++) {
                lcdDrawPixel(dev, x, y, color);
            }
        } else {
            for (int x = sect1; x <= sect4; x++) {
                lcdDrawPixel(dev, x, y, color);
            }
        }
...
// color:color
void lcdDrawRegularPolygon(TFT_t *dev, uint16_t xc, uint16_t yc, uint16_t n, uint16_t r, int16_t angle, colorSize color)
{
	double xd, yd, rd;
	int x1, y1;
	int x2, y2;
...
// r:radius
// color:color
void lcdDrawCircle(TFT_t * dev, uint16_t x0, uint16_t y0, uint16_t r, colorSize color) {
	int x;
	int y;
	int err;
	int old_err;
...
// r:radius
// color:color
void lcdDrawFillCircle(TFT_t * dev, uint16_t x0, uint16_t y0, uint16_t r, colorSize color) {
	int x;
	int y;
	int err;
	int old_err;
...
// r:radius
// color:color
void lcdDrawRoundRect(TFT_t * dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t r, colorSize color) {
	int x;
	int y;
	int err;
	int old_err;
...
void lcdDrawArrow(TFT_t * dev, uint16_t x0,uint16_t y0,uint16_t x1,uint16_t y1,uint16_t w,colorSize color) {
	double Vx= x1 - x0;
	double Vy= y1 - y0;
...
// w:Width of the botom
// color:color
void lcdDrawFillArrow(TFT_t * dev, uint16_t x0,uint16_t y0,uint16_t x1,uint16_t y1,uint16_t w,colorSize color) {
	double Vx= x1 - x0;
	double Vy= y1 - y0;
	double v = sqrt(Vx*Vx+Vy*Vy);
	//printf("v=%f\n",v);
...
// ascii: ascii code
// color:color
int lcdDrawChar(TFT_t * dev, FontxFile *fxs, uint16_t x, uint16_t y, uint8_t ascii, colorSize color) {
...
int lcdDrawCharBDF(TFT_t * dev, BDFChar * bdfChar, uint16_t x, uint16_t y, colorSize color) {
    uint16_t xx = x;
    uint16_t yy = y + bdfChar->height;

    for (int row = bdfChar->height - 1; row >= 0; row--) {
        uint32_t rowBitmap = bdfChar->bitmap[row];  // Each row's pixel data
        xx = x;  // Reset X for each new row
        for (int col = 0; col < bdfChar->width; col++) {
            if (rowBitmap & (1 << ((bdfChar->chars * 4) - 1 - col))) {  // Check each bit from left to right
                lcdDrawPixel(dev, xx, yy, color);
            }
            xx++;
        }
        yy--;  // Move to the next row UP
    }

    // Return next position (based on the character width)
    return x + bdfChar->width + 2;
}

int lcdDrawCharBDF2(TFT_t * dev, bdf_t * file_data, uint8_t ascii, uint16_t x, uint16_t y, colorSize color) {

    uint32_t index = file_data->index[ascii - 43];
    uint8_t width = file_data->chars[index];
    uint8_t height = file_data->chars[index + 1];
//    int8_t xOff = file_data->chars[index + 2];
    int8_t yOff = file_data->chars[index + 3];
    uint8_t length = file_data->lengths[ascii - 43];

//    ESP_LOGI("DRAW CHAR", "code %d", ascii);
//    ESP_LOGI("DRAW CHAR", "ind %lu", index);
//    ESP_LOGI("DRAW CHAR", "height %d", height);
...
//    ESP_LOGI("ST7", "index %lu", index);
//    ESP_LOGI("ST7", "width %d", width);
//    ESP_LOGI("ST7", "height %d", height);
//    ESP_LOGI("ST7", "length %d", length);

    for (int row = height; row > 0; row--) {
        uint32_t rowBitmap = file_data->chars[index + row + 3];  // Each row's pixel data
        xx = x;  // Reset X for each new row
        for (int col = 0; col < width; col++) {
            if (rowBitmap & (1 << ((length * 4) - 1 - col))) {  // Check each bit from left to right
                lcdDrawPixel(dev, xx, yy, color);
            }
            xx++;
        }
...
int lcdDrawCharBDF3(TFT_t * dev, bdf_t * file_data, uint8_t ascii, uint16_t x, uint16_t y, colorSize color) {
...
//    ESP_LOGI("FUCKING WHATEVER", "%c HEIGHT %d", (char) ascii, height);
...
int lcdDrawString(TFT_t * dev, const char * filepath, uint16_t x, uint16_t y, uint8_t * ascii, colorSize color) {
	int length = strlen((char *)ascii);
    BDFChar bdfChar;
	for(int i=0;i<length;i++) {
		if (dev->_font_direction == 0) {
            loadBDFChar(filepath, ascii[i], &bdfChar);
            x = lcdDrawCharBDF(dev, &bdfChar, x, y, color);
        }
	}
...
int lcdDrawString2(TFT_t * dev, bdf_t *file_data, BDFChar* bdfChar, uint16_t x, uint16_t y, uint8_t * ascii, colorSize color) {
    int length = strlen((char *)ascii);

    for(int i=0;i<length;i++) {
        if (dev->_font_direction == 0) {
            x = lcdDrawCharBDF2(dev, file_data, ascii[i], x, y + bdfChar->height, color);
        }
    }
    if (dev->_font_direction == 0) return x;
    return 0;
...
int lcdDrawString3(TFT_t * dev, bdf_t *file_data, BDFChar* bdfChar, uint16_t x, uint16_t y, uint8_t * ascii, colorSize color) {
    int length = strlen((char *)ascii);

    for(int i=0;i<length;i++) {
        if (dev->_font_direction == 0) {
            x = lcdDrawCharBDF3(dev, file_data, ascii[i], x, y + bdfChar->height, color);
        }
    }
    if (dev->_font_direction == 0) return x;
    return 0;
...
// code:character code
// color:color
int lcdDrawCode(TFT_t * dev, FontxFile *fx, uint16_t x,uint16_t y,uint8_t code,colorSize color) {
...
// utf8:UTF8 code
// color:color
int lcdDrawUTF8Char(TFT_t * dev, FontxFile *fx, uint16_t x,uint16_t y,uint8_t *utf8,colorSize color) {
	uint16_t sjis[1];
...
// utfs:UTF8 string
// color:color
int lcdDrawUTF8String(TFT_t * dev, FontxFile *fx, uint16_t x, uint16_t y, unsigned char *utfs, colorSize color) {
...
// Set font filling
// color:fill color
void lcdSetFontFill(TFT_t * dev, colorSize color) {
	dev->_font_fill = true;
...
// Set font underline
// color:frame color
void lcdSetFontUnderLine(TFT_t * dev, colorSize color) {
...
// r:radius
// color:color
void lcdSetCursor(TFT_t * dev, uint16_t x0, uint16_t y0, uint16_t r, colorSize color, uint16_t *save) {
	lcdGetRect(dev, x0-r, y0-r, x0+r, y0+r, save);
	lcdDrawCircle(dev, x0, y0, r, color);
}

void lcdResetCursor(TFT_t * dev, uint16_t x0, uint16_t y0, uint16_t r, colorSize color, uint16_t *save) {
...
void lcdDrawFinish(TFT_t *dev)
{
	...
	colorSize *image = dev->_frame_buffer;
	while (size > 0) {
		// 1024 bytes per time.
...
void getSlice(uint16_t *image, uint16_t *slice, int x_start, int y_start, int x_end, int y_end) {
    int slice_width = x_end - x_start + 1;
    int slice_height = y_end - y_start + 1;

    for (int y = 0; y < slice_height; y++) {
        for (int x = 0; x < slice_width; x++) {
            int src_index = (y_start + y) * CONFIG_WIDTH + (x_start + x);  // Calculate index in the original buffer
            int dst_index = y * slice_width + x;                    // Calculate index in the slice
            slice[dst_index] = image[src_index];
        }
    }
}

//void lcdDrawBlock(TFT_t *dev, int x_s, int y_s, int x_e, int y_e)
//{
//    spi_master_write_command(dev, 0x2A); // set column(x) address
//    spi_master_write_addr(dev, dev->_offsetx+x_s, dev->_offsetx+x_e-1);
//    spi_master_write_command(dev, 0x2B); // set Page(y) address
//    spi_master_write_addr(dev, dev->_offsety+y_s, dev->_offsety+y_e-1);
//    spi_master_write_command(dev, 0x2C); // Memory Write
//
//    uint32_t size = (x_e-x_s)*(y_e-y_s);
//    int slice_width = x_e - x_s;
//    int slice_height = y_e - y_s;
//
//    uint16_t *image = dev->_frame_buffer;
//    uint16_t *slice = (uint16_t*)malloc(slice_width * slice_height * sizeof(uint16_t));
//
//    getSlice(image, slice, x_s, y_s, x_e, y_e);
//
//    while (size > 0) {
//        // 1024 bytes per time.
//        uint16_t bs = (size > 1024) ? 1024 : size;
//        spi_master_write_colors(dev, slice, bs);
//        size -= bs;
//        image += bs;
//    }
//
